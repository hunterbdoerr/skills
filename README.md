...
- `implement-spec` (Codex only): Turn an orchestration-ready `spec.md` package into a human-approved plan and coordinate bounded implementation and independent verification.
...
`implement-spec` is Codex-only and is discovered directly at `.agents/skills/implement-spec`; it has no Claude Code or GitHub Copilot symlink. Ordinary specs remain flat at `specs/<year>/q<quarter>/<topic>.md`. Only an explicit request to prepare a spec for the `implement-spec` workflow uses `specs/<year>/q<quarter>/<topic>/spec.md`; `write-spec` creates only that `spec.md`, never implementation state.
